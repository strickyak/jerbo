/*  $Id: TranslationUnit.cpp,v 1.176 2021/11/13 20:37:52 sarrazip Exp $
...
TranslationUnit::emitGlobalVariables(ASMText &out, bool readOnlySection, bool withStaticInitializer) const
{
                                              ...
                                              jt != globalVariables.end(); jt++)
    {
        ...
        bool canBeStaticInit = (decl->initializationExpr && decl->initializationExpr->isStaticallyInitializable(*decl->getTypeDesc()));
        if (canBeStaticInit != withStaticInitializer)
            continue;

        if (withStaticInitializer)
        {
            if (decl->initializationExpr && decl->initializationExpr->definesOnlyAMatrixOfCharsAndHasInitializer(*decl->getTypeDesc()))  // e.g., char s[3][10] = { "foo", "bar", "baz" };
                decl->emitStaticArrayOfFixedSizeStringLiterals(out);
            else if (decl->initializationExpr && decl->initializationExpr->isArrayWithOnlyNumericalLiteralInitValues(*decl->getTypeDesc()))
...
{
    return targetPlatform == COCO_BASIC || targetPlatform == OS9;
}
