/*  $Id: Declaration.cpp,v 1.112 2021/11/13 20:20:45 sarrazip Exp $
...
CodeStatus
Declaration::emitStaticArrayOfFixedSizeStringLiterals(ASMText &out)
{
...
//
CodeStatus
Declaration::emitStaticValues(ASMText &out, const Tree *arrayElementInitializer, const TypeDesc *requiredTypeDesc)
{
        ...
        if (requiredTypeDesc->type == ARRAY_TYPE)
        {
            ...
            if (seq->size() < requiredNumArrayElements)
            {
                ...
                for (size_t i = seq->size(); i < requiredNumArrayElements; ++i)
                    if (!emitStaticValues(out, &wce, arrayElemTypeDesc/*, dimIndex + 1*/))
                        return false;
            }
...
//                   writingOffset is advanced by this parameter multiplied by the size of the array element.
    ...
    if (const TreeSequence *seq = dynamic_cast<const TreeSequence *>(initializer))
    {
        ...
        if (requiredTypeDesc->type == ARRAY_TYPE)
        {
            ...
            for (vector<Tree *>::const_iterator it = seq->begin(); it != seq->end(); ++it)
            {
                ...
                // arrayDimensions.back() must be used in the case of a character matrix, e.g., char a[2][6] = { "foo", "bar" };
                // This is covered by unit test "Character matrix" in test-program-output.pl.
                //
                uint16_t numSubArrayElements = arrayDimensions.size() > 0 ? arrayDimensions.back() : arrayElemTypeDesc->getNumArrayElements();
                if (!emitSequenceInitCode(out, *it, arrayElemTypeDesc, int16_t(numSubArrayElements), writingOffset))
                    return false;
            }
            return true;
        }
...
Declaration::checkArrayInitializer(Tree *initializationExpr, const TypeDesc *varTypeDesc, const string &variableId, const vector<uint16_t> &arrayDimensions, size_t dimIndex)
{
    ...
    if (seq != NULL)
    {
        ...
        if (seq->size() < size_t(numArrayElements))
        {
            if (varTypeDesc->pointedTypeDesc->isArray() || varTypeDesc->pointedTypeDesc->isStruct())
                initializationExpr->errormsg("only %u element(s) in initializer for array of %hu element(s) (not supported when elements are arrays or structs)",
                                            seq->size(), numArrayElements);
            else
